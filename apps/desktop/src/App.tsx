...
export default function App() {
  ...
  useEffect(() => {
    ...
    loadSecrets().then((secrets) => {
      ...
      // non-empty value here was typed before the keychain answered. Keep that edit and
      // persist it instead of clobbering it with the stored value.
      const typed = settingsRef.current;
      const apiKey = typed.apiKey !== "" ? typed.apiKey : secrets.apiKey;
      const hfToken = typed.hfToken !== "" ? typed.hfToken : secrets.hfToken;
      ...
      setSettings((current) => ({ ...current, apiKey, hfToken }));
    });
    return () => {
      cancelled = true;
    };
